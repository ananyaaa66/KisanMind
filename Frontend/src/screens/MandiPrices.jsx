...
export default function MandiPrices() {
  const { lang, advisoryData, preferences } = useApp()
  ...
  const formatPrice = (priceInINR) => {
    ...
    if (preferences?.currency === 'USD') {
      const priceInUSD = Math.round(priceInINR / 83)
  ...
  const actionReason = liveMarket
    ...
    : predictedPrice
      ? (lang === 'hi'
        ? `XGBoost मॉडल: 7 दिन में ${formatPrice(Math.round(predictedPrice))} अनुमानित। ${Number(trendPercent) > 0 ? 'बढ़त' : 'गिरावट'} का रुझान।`
        : `XGBoost Model: ${formatPrice(Math.round(predictedPrice))} predicted in 7 days. ${Number(trendPercent) > 0 ? 'Upward' : 'Downward'} trend.`)
      : ''
  ...
  return (
    <Screen title={t('pricesTitle', lang)} subtitle="Agent 2">
      <div className="lg:grid lg:grid-cols-12 lg:gap-6 lg:space-y-0">
        {/* Left Column: Crop selection & Price display */}
        <div className="lg:col-span-7 space-y-4">
          ...
          {/* Big price display */}
          <div className="glass active p-5 text-center relative overflow-hidden">
            {liveMarket && (
            ...
            <div>
              ...
              <div className="glass p-3 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-[var(--text-dim)]">{formatPrice(liveMarket.current_price_per_quintal)}/qtl</p>
                </div>
        ...
        {/* Right Column: XGBoost Forecast & Trends */}
        <div className="lg:col-span-5 space-y-4 mt-4 lg:mt-0">
          ...
          {predictedPrice && !loading && (
            <div className="glass p-4 bg-gradient-to-r from-crop/10 to-lime/10 border border-crop/30">
              ...
              <div className="flex items-end justify-between">
                <div>
                  <p className="num text-2xl font-bold text-lime">{formatPrice(predictedPrice)}</p>
          ...
          <div className="grid grid-cols-2 gap-3">
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="glass p-3">
              ...
              <p className="text-lg font-bold text-lime">
                {predictedPrice ? formatPrice(predictedPrice) : '—'}
              </p>
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="glass p-3">
              ...
              <div className="flex items-center gap-1">
                ...
                <p className="text-sm font-semibold capitalize text-white">{priceTrend}</p>
              </div>
