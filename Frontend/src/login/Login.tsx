...
export default function Login({ onLogin }: { onLogin?: () => void } = {}) {
  ...
  useEffect(() => {
    const API = import.meta.env.VITE_API_URL || "https://kisanmind-ls1d.onrender.com";
    fetch(`${API}/stats`)
      .then(res => res.json())
      .then(data => {
        if (data.success) {
    ...
    >
      ...
      >
        ...
        <div className="relative z-10">
          ...
          <div className="mb-12">
            <h1
              className="text-[38px] font-semibold leading-[1.15] tracking-tight mb-4"
              style={{ color: "#f0f7eb" }}
            >
              Agricultural<br />Intelligence<br />Platform
            </h1>
            <p className="text-[14px] leading-relaxed" style={{ color: "#8aab78", maxWidth: 340 }}>
              Crop advisory, market forecasting, and government scheme management
              for Uttar Pradesh agricultural officers and cooperatives.
            </p>
          </div>
        ...
        <div className="w-full" style={{ maxWidth: 360 }}>
          ...
          <div className="mb-8">
            ...
            <p className="text-[13px] text-[#737373] mt-1.5">
              {mode === 'register' ? 'Join the agricultural intelligence portal' : 'Access the agricultural intelligence portal'}
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <>
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="FK-UP-2024-XXXX or email"
                    required
                    className="w-full px-3 py-2.5 text-[13.5px] text-[#111] bg-white border border-[#e5e5e5] outline-none transition-colors placeholder:text-[#c0c0c0]"
            ...
            {mode === 'login' && (
              <div className="flex items-center gap-2.5 pt-1">
                <button
                  ...
                  className={`w-4 h-4 border flex items-center justify-center transition-colors flex-shrink-0 ${remember ? "bg-[#2d5a1b] border-[#2d5a1b]" : "bg-white border-[#d4d4d4]"}`}
                  style={{ borderRadius: 2 }}
                >
                  {remember && (
            ...
            <div className="pt-2">
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-2.5 text-[13.5px] font-semibold text-white transition-opacity disabled:opacity-60"
                style={{ backgroundColor: "#1a3310", borderRadius: 2 }}
                onMouseEnter={(e) => !loading && (e.currentTarget.style.backgroundColor = "#2d5a1b")}
                onMouseLeave={(e) => !loading && (e.currentTarget.style.backgroundColor = "#1a3310")}
              >
                {loading ? (
                  <>
                    <span
          ...
          {mode === 'login' && (
            <>
              ...
              <p className="mt-4 text-center text-[13px] text-[#737373]">
                Don't have an account? <button onClick={() => { setMode('register'); setError(""); }} className="text-[#2d5a1b] font-medium hover:underline">Register here</button>
              </p>
            </>
          )}
          ...
          {mode === 'register' && (
            <p className="text-center text-[13px] text-[#737373]">
              Already have an account? <button onClick={() => { setMode('login'); setError(""); }} className="text-[#2d5a1b] font-medium hover:underline">Sign in here</button>
            </p>
          )}

          {/* Footer */}
          <p className="mt-8 text-center text-[11.5px] text-[#a3a3a3] leading-relaxed">
            For access requests, contact your District Agricultural Officer.<br />
            <span className="text-[#c0c0c0]">KisanMind · UP Directorate of Agriculture · 2026</span>
          </p>
        </div>
      </div>
